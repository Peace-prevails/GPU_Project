...
void runNBodySimulationCPU(Body *bodies, int n) {
    clock_t start, end;
    start = clock();
...
void runNBodySimulationSingleGPU(Body *h_bodies, int n) {
    ...
    printf("Single GPU overhead time: %f m ms\n", milliseconds1);
...
int main(int argc, char * argv[])
{
  ...
  {
    case 0: printf("CPU version:\n");
        // clock_t start, end;
        // start = clock();
        runNBodySimulationCPU(h_bodies, n);
        
      break;

	case 1: printf("Single GPU version:\n");
      runNBodySimulationSingleGPU(h_bodies, n);
      break;


		
	case 2: printf("Multi GPU version:\n");
